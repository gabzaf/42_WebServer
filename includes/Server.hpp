...
class	Server
{
	private:
		...
		void	handleGet(int fd, const std::string &path, const std::string &uri, const LocationConfig *loc,
				const HttpRequest &req,	const ServerConfig &config, bool headOnly = false);
		void	handlePost(int fd, const HttpRequest &req, const LocationConfig *loc, const ServerConfig &config);
		void	handleDelete(int fd, const std::string &path, const LocationConfig *loc, const ServerConfig &config);
		void	handleCgi(int fd, const HttpRequest &req, const LocationConfig *loc, const std::string &path,
				const ServerConfig &config, bool headOnly = false);
		void	handleRedirection(int fd, const std::pair<int, std::string> &redirection, const ServerConfig &config);

		void	serveFile(int fd, const std::string &path, const ServerConfig &config, bool headOnly = false);
		void	sendError(int fd, int code, const ServerConfig &config);

		std::string	getStatusMsg(int code);
		std::string	getContentType(const std::string &path);
		
		void	generateAutoindex(int fd, const std::string &path, const std::string &uri, const ServerConfig &config,
				bool headOnly = false);
		std::string	resolvePath(const std::string &path, const LocationConfig *loc, const ServerConfig &config);
