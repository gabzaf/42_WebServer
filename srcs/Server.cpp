...
void	Server::run()
{
	while (true)
	{
		...
		for (std::map<int, size_t>::iterator it = _last_activity.begin(); it != _last_activity.end();)
		{		
...
const	ServerConfig &Server::matchServer(int client_fd, const std::string &host)
{
	struct sockaddr_in	addr;
...
void	Server::handleRedirection(int fd, const std::pair<int, std::string> &redirection, const ServerConfig &config)
{
	(void)config;
...
void	Server::generateAutoindex(int fd, const std::string &path, const std::string &uri,
		const ServerConfig &config, bool headOnly)
{
...
void	Server::handleCgi(int fd, const HttpRequest &req, const LocationConfig *loc, const std::string &path,
		const ServerConfig &config, bool headOnly)
{
...
void	Server::handlePost(int fd, const HttpRequest &req, const LocationConfig *loc, const ServerConfig &config)
{
	std::string	targetPath = resolvePath(req.getPath(), loc, config);
	if (config.client_max_body_size > 0 && req.getBody().size() > config.client_max_body_size)
	{
...
void	Server::handleDelete(int fd, const std::string &path, const LocationConfig *loc, const ServerConfig &config)
{
	(void)loc;
	struct stat s;
...
const LocationConfig	*Server::matchLocation(const std::string &path, const ServerConfig &config)
{
	...
	for (size_t i = 0; i < config.locations.size(); ++i)
	{
		...
		else if (locPath.size() > 1 && locPath[locPath.size() - 1] == '/' && path + "/" == locPath)
		{
			if (locPath.length() > bestPrefixLength)
			{
				bestPrefixLength = locPath.length();
...
bool	Server::isMethodAllowed(const std::string &method, const LocationConfig *loc)
{
...
void	Server::serveFile(int fd, const std::string &path, const ServerConfig &config, bool headOnly)
{
